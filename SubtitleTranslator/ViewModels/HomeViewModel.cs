...
namespace SubtitleTranslator.ViewModels
{
	public class HomeViewModel : TranslationViewModelAbstract
	{
...
#if ANDROID
			bool b = await HasStorageWritePermissionsAsync();
			//bool b=true;
			if (b){
				var dir = Android.App.Application.Context.GetExternalFilesDir(Android.OS.Environment.DirectoryDownloads);
				await SaveFiles(dir.AbsolutePath);
				//var dir = Android.OS.Environment.GetExternalStoragePublicDirectory(Android.OS.Environment.DirectoryDocuments);
				//await SaveFiles(dir.AbsolutePath);
				//var dir = Android.OS.Environment.DirectoryMovies;
				//await SaveFiles(Path.Combine(dir.AbsolutePath,Android.OS.Environment.DirectoryDocuments));
			}
#else
		...
		private async Task<bool> HasStorageWritePermissionsAsync()
		{
